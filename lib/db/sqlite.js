...
module.exports = utils.inherit(Object, {
  ...
  getAssignmentData: function(params, fn) {
    this.connection.get(templates.getAssignmentData, [params.assignNumber],
        function(err, result) {
          if (err) {
            return fn(err);
          }
  ...
  getStudentAssignments: function(params, fn) {
    this.connection.all(templates.getStudentAssignments, [params.user.name],
        function(err, result) {
          fn(err, result);
        });
  },
  setAssignmentStarted: function(username, assignment_id, bin, fn) {
    var sql = templates.setAssignmentStarted;
    this.connection.run(sql, [username, assignment_id, bin.url], fn);
  },
