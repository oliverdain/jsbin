...
module.exports = Observable.extend({
  ...
  startAssignment: function(req, res, next) {
    console.log('startAssignment called!');
    if (!req.session.user) {
    ...
    var setUserForAssignment = function(err, bin) {
      ...
      // TODO(NOW) This needs to:
      // 1) call setBinOwner
      // 2) call a method (not yet written) that put this assignment
      //    in the assignments table.
      console.log('Setting %s as owner of assignment %s',
          req.session.user.name, req.params.number);
