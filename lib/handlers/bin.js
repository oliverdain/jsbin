...
module.exports = Observable.extend({
  ...
  getStudentHome: function(req, res, next) {
    this.models.bin.getStudentAssignments({user: req.session.user},
        function(err, result) {
          if (err) {
            res.send('Bummer:\n%s', err);
          } else {
            ...
            for (i = 0; i < result.length; ++i) {
              if (result[i].url === null) {
                notStarted.push(result[i]);
              } else if (result[i].revision === null) {
                started.push(result[i]);
              } else {
                done.push(result[i]);
              }
            }
            data = {
              ...
              assignments: {not_started: notStarted,
                started: started, done: done}
            };
            res.render('student_home', data);
          }
        });
  },
  ...
  startAssignment: function(req, res, next) {
    console.log('startAssignment called with: %s', req.body.assignNumber);
    if (!req.session.user) {
      console.error('startAssignment called with no valid user.');
      return next(new errors.NotFound(
            'User must be logged in to start an assignment'));
    }
    var _this = this;


    // 2nd callback
    var setUserForAssignment = function(err, bin) {
      if (err) {
        return next(err);
      }
      bin.summary = 'Assignment ' + req.body.assignNumber;

      var renderCreatedCb = function(err, owners_id) {
        if (err) {
          return next(err);
        } else {
      ...
      var setOwnerCb = _.partial(_this.models.user.setBinOwner,
          req.session.user.name, bin, renderCreatedCb);
      var setOwnerCb = _.bind(setOwnerCb, _this.models.user);
      _this.models.user.setAssignmentStarted(req.session.user.name,
          req.body.assignNumber, bin, setOwnerCb);
    }

    // 1st callback.
    var createBin = function(err, binData) {
      if (err) {
        return next(err);
      }
      if (!binData) {
        return next(new errors.BadRequest('No such assignment'));
      }

      _this.models.bin.create(binData, setUserForAssignment);
    }

    this.models.bin.getAssignmentData(req.body, createBin);
  },
